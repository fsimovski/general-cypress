...
export const patchPostsFunction = (posts: String, number: String) => {
  cy.request('PATCH', '/' + posts + number, postsRequestModel)
    .its("status")
    .then((statusCode) => {
      expect(statusCode).to.equals(200)
    })

  return cy.request('PATCH', '/' + posts + number, postsRequestModel)
    .its("body")
    .then((response) => {
      ...
      expect(response.id).to.equals(postsRequestModel.id, ' "id" parameter should be ' + '"' + (response.id).toString() + '"')
